...
export const PaymentOption = (props: Props) => {
  ...
  //今月繰り越す金額
  const [carryOverPrice, setCarryOverPrice] = useState<number>(
    billingData.carryOverPrice,
  );
  ...
  const updateBillingData = async (type: string, price: number) => {
    const newBillingData: BillingData = {
      ...
      useAmount: billingData.useAmount,
      price: billingData.price,
      beforeCarryOver: billingData.beforeCarryOver,
      carryOverType: type,
      carryOverPrice: price,
      dateId: billingData.dateId,
      paidPrice: billingData.paidPrice,
  ...
  if (carryOverType === 'no') {
    return (
      <div>
        ...
        >
          <DialogTitle id="alert-dialog-title">{'繰越設定完了'}</DialogTitle>
          <DialogContent>
        ...
        <Stack spacing={2}>
          <Button
            onClick={() =>
              updateBillingData(
                'all',
                billingData.price + billingData.beforeCarryOver,
              )
            }
            disabled={saveDisabled}
          >
